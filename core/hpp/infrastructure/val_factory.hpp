...
template <typename IAllocVal> struct val_factory {
    val_factory(IAllocVal& alloc_val);
...
val_factory<IAllocVal>::val_factory(IAllocVal& alloc_val)
    : alloc_val_(alloc_val) {
}

template <typename IAllocVal>
std::shared_ptr<val> val_factory<IAllocVal>::make_clo(std::shared_ptr<expr> term,
                                                     std::shared_ptr<env> e) {
    return alloc_val_.alloc(val{val::clo{std::move(term), std::move(e)}});
}

template <typename IAllocVal>
std::shared_ptr<val> val_factory<IAllocVal>::make_fvar(uint32_t depth) {
    return alloc_val_.alloc(val{val::fvar{depth}});
}

template <typename IAllocVal>
std::shared_ptr<val>
val_factory<IAllocVal>::make_napp(std::shared_ptr<val> head,
                                  std::shared_ptr<expr> arg,
                                  std::shared_ptr<env> arg_env) {
