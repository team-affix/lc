...
struct RuntimePlaygroundTest : public ::testing::Test, public nbe_fixture {
    struct drive {
        ...
        drive(std::shared_ptr<expr> term, rc_pool<expr>& out_nodes)
            : rt(std::move(term), out_nodes), steps(0), ms(0), out() {
...
TEST_F(RuntimePlaygroundTest, SKKIsIdentity) {
    // S K K → λx. x
    drive d{ap(ap(s_term(), k_term()), k_term()), out_nodes};
    note("S K K", d);
...
TEST_F(RuntimePlaygroundTest, SKSIIsIdentity) {
    ...
    drive d{ap(ap(s_term(), ki), id_term()), out_nodes};
    note("S (K I) I", d);
    EXPECT_TRUE(exprs_eq(d.out, id_term()));
}

TEST_F(RuntimePlaygroundTest, ChurchPlusThreeFour) {
    drive d{ap(ap(plus_comb(), church(3)), church(4)), out_nodes};
    note("plus 3 4", d);
    EXPECT_TRUE(exprs_eq(d.out, church(7)));
}

TEST_F(RuntimePlaygroundTest, ChurchTimesFiveSix) {
    drive d{ap(ap(times_comb(), church(5)), church(6)), out_nodes};
    note("times 5 6", d);
...
TEST_F(RuntimePlaygroundTest, ChurchExpTwoFive) {
    // 2^5 = 32
    drive d{ap(ap(exp_comb(), church(2)), church(5)), out_nodes};
    note("exp 2 5", d);
...
TEST_F(RuntimePlaygroundTest, ChurchExpThreeFour) {
    ...
    drive d{ap(ap(exp_comb(), church(3)), church(4)), out_nodes};
    note("exp 3 4", d);
    EXPECT_TRUE(exprs_eq(d.out, church(81)));
}

TEST_F(RuntimePlaygroundTest, PredOfSeven) {
    drive d{ap(pred_comb(), church(7)), out_nodes};
    note("pred 7", d);
...
TEST_F(RuntimePlaygroundTest, NestedKDiscardsJunk) {
    ...
    drive d{ap(ap(k_term(), keep), junk), out_nodes};
...
TEST_F(RuntimePlaygroundTest, FibSeven) {
    // fib 7 = 13
    drive d{ap(fib_comb(), church(7)), out_nodes};
    note("fib 7", d);
...
TEST_F(RuntimePlaygroundTest, FactBinIterSix) {
    ...
    drive d{ap(fact_bin_iter_comb(), church(6)), out_nodes};
...
TEST_F(RuntimePlaygroundTest, BooleanXorViaChurch) {
    ...
    drive d{ap(ap(or_comb(), left), right), out_nodes};
    note("xor T F", d);
...
TEST_F(RuntimePlaygroundTest, WCombinatorDuplicates) {
    ...
    drive d{ap(ap(w_term(), times_comb()), church(3)), out_nodes};
    note("W times 3", d);
