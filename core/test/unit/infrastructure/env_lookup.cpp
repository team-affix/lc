...
TEST_F(EnvLookupTest, IndexZeroReturnsSameCell) {
    env* e = envs.make_delayed(pool.make_var(0), nullptr, nullptr);
    EXPECT_EQ(lookup.lookup(e, 0), e);
}

TEST_F(EnvLookupTest, IndexOneReturnsParent) {
    env* outer = envs.make_delayed(pool.make_var(1), nullptr, nullptr);
    env* inner = envs.make_delayed(pool.make_var(0), nullptr, outer);
