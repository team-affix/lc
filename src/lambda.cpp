...
void var::print(std::ostream &a_ostream) const { a_ostream << m_index; }
...
std::unique_ptr<expr>
var::substitute(size_t a_lift_amount, size_t a_var_index,
...
std::unique_ptr<expr>
func::substitute(size_t a_lift_amount, size_t a_var_index,
                 const std::unique_ptr<expr> &a_arg) const {
...
std::unique_ptr<expr>
app::substitute(size_t a_lift_amount, size_t a_var_index,
                const std::unique_ptr<expr> &a_arg) const {
...
// EXPR CLONE METHOD
std::unique_ptr<expr> expr::clone() const { return lift(0, 0); }
...
void test_func_lift() {
  // local body, lift 1 level
  {
    ...
    const var *l_lifted_var = dynamic_cast<var *>(l_lifted_func->m_body.get());
    assert(l_lifted_var != nullptr);
    assert(l_lifted_var->m_index == 1);
  }
  ...
  // f(f((1 3 6))), lift 2, cutoff 3
  {
    ...
    // l(3) >= 3, lifted to 5
    // l(6) >= 3, lifted to 8
    auto l_expected = f(f(a(a(v(1)->clone(), v(5)->clone()), v(8)->clone())));
    assert(l_lifted->equals(l_expected));
  }
...
void test_app_lift() {
  ...
  {
    auto l_app =
        a(a(a(a(v(1)->clone(), v(2)->clone()), v(3)->clone()), v(4)->clone()),
          v(5)->clone());
    ...
    auto l_expected =
        a(a(a(a(v(1)->clone(), v(2)->clone()), v(7)->clone()), v(8)->clone()),
          v(9)->clone());
    assert(l_lifted->equals(l_expected));
  }
...
void test_func_substitute() {
  // single composition lambda, outer depth 0, occurrance found
  {
    ...
    // get body
    const var *l_subbed_var = dynamic_cast<var *>(l_subbed_func->m_body.get());
  ...
  // Tests that locals < var_index are left alone
  {
    ...
    auto l_body =
        a(a(a(a(v(0)->clone(), v(1)->clone()), v(2)->clone()), v(3)->clone()),
          v(4)->clone());
    auto l_func = f(l_body->clone());
    auto l_sub = v(99);
    ...
    auto l_expected = f(
        a(a(a(a(v(0)->clone(), v(1)->clone()), v(2)->clone()), v(100)->clone()),
          v(3)->clone()));
  ...
  // func with nested func, testing locals < var_index preservation
  {
    ...
    auto l_expected = f(f(a(a(v(0)->clone(), v(90)->clone()), v(2)->clone())));
...
void test_app_substitute() {
  // app of locals, both are occurrances
  {
    ...
    // get lhs
    const var *l_subbed_lhs = dynamic_cast<var *>(l_subbed_app->m_func.get());
  ...
  // app of funcs, both with occurrances
  {
    ...
    // get lhs
    const func *l_subbed_lhs = dynamic_cast<func *>(l_subbed_app->m_func.get());

    // make sure lhs is a func
    assert(l_subbed_lhs != nullptr);

    // get rhs
    const func *l_subbed_rhs = dynamic_cast<func *>(l_subbed_app->m_arg.get());
  ...
  // (λ.(0 1 2) λ.(1 2 3)) with var 2 -> l(55)
  {
    ...
    // l(2) matches var 2, replaced with l(55) lifted by 1 = l(56)
    // l(3) decrements to l(2)
    auto l_expected = a(f(a(a(v(0)->clone(), v(1)->clone()), v(56)->clone())),
...
void generic_use_case_test() {
  ...
  // test the church bools
  {
    ...
    const auto l_true_program =
        construct_program(l_helpers.begin(), l_helpers.end(), l_true_case_main);
  ...
  // test succ church numerals
  {
    ...
    assert(
        FOUR_REDUCED->equals(f(f(a(g(0), a(g(0), a(g(0), a(g(0), g(1)))))))));
  ...
  l_helpers.emplace_back(f(f(f(f(a(a(l(0), l(2)), a(a(l(1), l(2)), l(3))))))));
  ...
  // test add church numerals
  {
    ...
    // add three and two
    const auto ADD_THREE_TWO = a(a(ADD->clone(), THREE->clone()), TWO->clone());

    // add five and five
    const auto ADD_FIVE_FIVE = a(a(ADD->clone(), FIVE->clone()), FIVE->clone());
    ...
    const auto ADD_THREE_TWO_PROGRAM =
        construct_program(l_helpers.begin(), l_helpers.end(), ADD_THREE_TWO);
    const auto ADD_FIVE_FIVE_PROGRAM =
        construct_program(l_helpers.begin(), l_helpers.end(), ADD_FIVE_FIVE);
