...
| `frame` | C++ program frames (`reduce_*` / `reify_*`) on a heap `deque` |
| `reducer` / `reifier` | `process_*` returns optional child frame |
| `processor` | Hub: `process(frame&)` routes via `get_if` to reducer/reifier |
| `interpreter` | `step` / `done`; constructed with processor + initial frame |
| `normalizer` | `make_clo(term, nil)` then returns root `reify_val` frame |
...
**LC env vs C++ program frames:** `env_pool::make_env` builds lambda-calculus environment cells. Program frames are separate CEK-style control records.

## Layout

```
core/
  hpp/value_objects/     expr, val, env, program frames
...
processor<decltype(red), decltype(red), decltype(red), decltype(re),
          decltype(re), decltype(re)>
    proc(red, red, red, re, re, re);
normalizer<val_pool> norm(vals);

const expr* id = pool.make_abs(pool.make_var(0));
const expr* nf;
interpreter<frame, decltype(proc)> interp(proc, norm.normalize(nf, pool.make_app(id, id)));
while (interp.step()) {
}
if (interp.done()) {
